...
class BarrageViewGroup : ConstraintLayout {
    ...
    fun addBarrageView(contentList: ArrayList<String>) {
        ...
        for ((index, value) in contentList.withIndex()) {
            when ((index + 1) % 3) {
                1 -> {
                    initTextView(value, BarrageLocation.TOP)
                }
                2 -> {
                    initTextView(value, BarrageLocation.CENTER)
                }
                0 -> {
                    initTextView(value, BarrageLocation.BOTTOM)
                }
            }
        }
    }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        for (i in 0 until childCount / 2) {
            ...
            if (themaxChildWidth <= topMarginLeft) {
                themaxChildWidth = topMarginLeft
            }
            if (themaxChildWidth <= centerMarginLeft) {
                themaxChildWidth = centerMarginLeft
            }
            if (themaxChildWidth <= bottomMarginLeft) {
                themaxChildWidth = bottomMarginLeft
            }
            var randomNumber = (Math.random() * (100 - 50) + 50).roundToInt().toInt()
            when ((i + 1) % 3) {
                1 -> {
                    textViewList[i].leftDistance = topMarginLeft
                    textViewList_two[i].leftDistance = topMarginLeft
                    topMarginLeft += childWidth + randomNumber
                }
                2 -> {
                    textViewList[i].leftDistance = centerMarginLeft
                    textViewList_two[i].leftDistance = centerMarginLeft
                    centerMarginLeft += childWidth + randomNumber
                }
                0 -> {
                    textViewList[i].leftDistance = bottomMarginLeft
                    textViewList_two[i].leftDistance = bottomMarginLeft
                    bottomMarginLeft += childWidth + randomNumber
                }
            }
            textViewList[i].childWidth = childWidth
            textViewList_two[i].childWidth = childWidth
        }
        if (!startAnimal) {
            startAnimal = true
            startAnimalList_One()
    ...
    private fun initTextView(content: String, location: BarrageLocation) {
    ...
    private fun initTextViewStyle(content: String, location: BarrageLocation): TextView {
        ...
        when (location) {
            ...
            BarrageLocation.CENTER -> {

                layoutParams.topToTop = ConstraintLayout.LayoutParams.PARENT_ID
                layoutParams.bottomToBottom = ConstraintLayout.LayoutParams.PARENT_ID
                layoutParams.leftToRight = ConstraintLayout.LayoutParams.PARENT_ID
            }
            BarrageLocation.BOTTOM -> {
                layoutParams.bottomToBottom = ConstraintLayout.LayoutParams.PARENT_ID
                layoutParams.leftToRight = ConstraintLayout.LayoutParams.PARENT_ID
            }
        }
    ...
    private fun startAnimalList_One() {
        if (textViewList_two.size > 0) {
            ...
            animal?.addUpdateListener { valueAnimator ->
                ...
                if (vcango && abs(hanRunDistance) > themaxChildWidth) {
    ...
    private fun startAnimalList_Two() {
        if (textViewList_two.size > 0) {
            ...
            animal?.addUpdateListener { valueAnimator ->
                ...
                if (vango && abs(hanRunDistance) > themaxChildWidth) {
    ...
    private fun createDropAnimator(itemViewBean: ItemViewBean, listcontent: List<ItemViewBean>) {
        ...
        var translationX: ObjectAnimator = ObjectAnimator.ofFloat(itemViewBean.view, "translationX", 0f, -(runDistance))
        itemViewBean.objectAnimator = translationX
        var ll = itemViewBean.view.layoutParams as LayoutParams
        ll.leftMargin = itemViewBean.leftDistance
        itemViewBean.view.layoutParams = ll

...
public class SS extends View {
    ...
    public SS(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public SS(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                if (mDrawMoveHistory.size() > 0) {
                    if (upNum == mDrawMoveHistory.size() - 1) {
    ...
    private void addNewPath(MotionEvent event) {
        ...
        Log.i("zjc", "event.getActionIndex() : " + event.getActionIndex() + "   event.getPointerId: " + event.getPointerId(event.getActionIndex()));
